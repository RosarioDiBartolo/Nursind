...
def test_turni_afternoon_long_export_process_one_contract(tmp_path: Path) -> None:
    ...
    result = process_one_enriched_file(source, output_dir=str(output_dir), pairs_dir=str(pairs_dir))
    ...
    assert out_csv == output_dir / "Mario Rossi" / "Mario Rossi.pomeriggi.csv"
    ...
    pairs_csv = Path(str(result["output_pairs_csv"]))
    assert pairs_csv == output_dir / "Mario Rossi" / "Mario Rossi.csv"
...
def test_turni_afternoon_long_export_process_many_contract(tmp_path: Path) -> None:
    ...
    _write_pairs_csv(pairs_dir / "Mario Rossi.pairs.csv", [{"duration_hhmm": "08:00"}])
    _write_pairs_csv(pairs_dir / "Anna Bianchi.pairs.csv", [{"duration_hhmm": "04:00"}])
