...
| `src/raw_text_parsing.py` | Shared raw-text parsing primitives and regexes for text->days/events flow | raw parsing, regex parsing, day extraction, event extraction | In: raw extracted text. Out: parsed days/events structures | `src/extract_days_from_text_raw/service.py`, `src/extract_days_from_text_raw/parsers/common.py`, `src/extract_events_from_days_raw/service.py` | Duplicate regex/date/event parsing primitives |
| `src/extract_days_from_text_raw/parsers/` | Extensible day-parser platform (`BaseFormatParser`, auto-discovery loader, deterministic router, per-format parser modules) | parser registry, parser auto-discovery, routing score, cartellino/timbrature strategies | In: full document text + day row raw lines. Out: selected parser + `mo_f/mo_t/mo_lav` values | `src/extract_days_from_text_raw/service.py` | Hard-coded `if/elif` parser routing and format-specific extraction logic in service |
