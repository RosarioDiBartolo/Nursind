...
from core.shift_logic import to_bool_series
...
def export_afternoon_long_from_dir(
    *,
    ...
    if not enriched_files:
        message = (
            f"No enriched CSV files found in {enriched_path.resolve()}. "
            "Run src/scripts/enrich_shifts.py before the afternoon export."
        )
        report = build_stage_report(
            stage=TURNI_AFTERNOON_LONG_EXPORT_ARTIFACTS.step,
            status="error",
            inputs={
                "enriched_dir": str(enriched_path.resolve()),
                "pairs_dir": str(Path(pairs_dir).resolve()),
                "output_dir": str(Path(output_dir).resolve()),
                "filter": {"is_afternoon": True, "is_long": True},
            },
            outputs={
                "output_dir": str(Path(output_dir).resolve()),
                "report_json": str(Path(report_json).resolve()),
            },
            stats={
                "files_total": 0,
                "files_processed": 0,
            ...
            row_totals={"items": 0, "issues": 1, "filtered_files": 0},
            issues=[
                {
                    "code": "missing_enriched_inputs",
                    "enriched_dir": str(enriched_path.resolve()),
                    "message": message,
                }
            ],
