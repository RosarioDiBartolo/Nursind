...
class PairsCloser:
    ...
    def close(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.mark_inferred:
            closed_rows = closed_rows.assign(closed_inferred=False)
...
class ShiftClassifier:
    ...
    def classify(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        working = working.assign(
            duration=working["exit_ts"] - working["entry_ts"],
            anno=working["entry_ts"].dt.year,
        )
        ...
        working = working.assign(
            ...
            is_night=self._match(turno_norm, "notte"),
            is_afternoon=self._match(turno_norm, "pomeriggio"),
        )
        ...
        return working.assign(is_holiday=is_holiday)
