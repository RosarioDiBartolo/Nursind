...
def format_output_pairs(df: pd.DataFrame, *, keep_inferred_column: bool) -> pd.DataFrame:
    ...
    working.loc[:, "duration_hhmm"] = _duration_hhmm(working["entry_ts"], working["exit_ts"])
    working.loc[:, "turno"] = _turno_from_entry(working["entry_ts"])

    working = working.assign(
        entry_ts=working["entry_ts"].dt.strftime("%Y-%m-%d %H:%M:%S"),
        exit_ts=working["exit_ts"].dt.strftime("%Y-%m-%d %H:%M:%S"),
    )
