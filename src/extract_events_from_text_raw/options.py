...
if build_pipelines_paths is not None:
    ...
    DEFAULT_REPORT_JSON = str(
        Path(DEFAULT_OUTPUT_DIR) / "extract_events_from_text_raw.report.json"
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Parse canonical extracted document payloads and generate per-file raw events CSV outputs."
    )
    parser.add_argument(
        "--input-dir",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(e.g. source.pdf -> source.events_from_text_raw.csv)"
        ),
    )
    parser.add_argument(
        "--report-json",
