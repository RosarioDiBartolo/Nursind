...
- **Drive Filter**: `python -m drive_scanner.fetch_index --index index.json --out output --report report.json -v`
...
- `src/drive_scanner/fetch_index.py`: Filters/validates PDFs and writes parsed outputs
- `tests/test_fetch_index_merge.py`: Merge behavior for filtered indexs
...
- **ZIP Handling**: Drive scan includes ZIP files in indexs; filtering can decide how to handle them
...
- **Index Schema**: `root_id`, `generated_at`, `employee_count`, `employees[]` with `included`, `skipped`, `excluded_folders`, `counts`
